...
- 本地 `B1_proxy_score` 不是平台截图中的真实 B1 总分。
- 尚未模拟涨跌停无法成交、盘口滑点、停牌、真实容量和更细交易费用。
- 活跃市值 Top300 使用 `2024-12-31` as-of 股票池，后续若做生产化，需要滚动构建股票池，避免固定池带来的样本选择偏差。
- 当前结论只证明本地研究 goal 的回测闸门通过，不构成实盘投资建议。

下一步只有两条值得继续：

1. 校准平台未公开的 B1 总分与“更多卖出规则”，否则本地代理很可能选错候选或退出点。
2. 把当前质量过滤候选做更严实盘约束验证：涨跌停成交、停牌、滚动活跃池、容量、滑点和更细 walk-forward。
