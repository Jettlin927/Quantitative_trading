...
- 在选股池中查看技术评分、基本面评分、PE/PB、市值、换手率、ROE、毛利率、负债率、净利同比和形态标签。
- 查看财联社、华尔街见闻、雪球热榜等真实消息源，不伪造新闻。
- 在策略实验页切换 BOLL、MACD、均线、RSI 等预设策略，并用交互式 K 线查看 MA、BOLL、MACD、成交量和买卖点。
- 调用 DeepSeek 生成策略复盘；没有 `DEEPSEEK_TOKEN` 时会降级为本地规则评价。
...
7. 进入“策略实验”，调整策略参数并运行回测。
8. 在“AI复盘”查看 DeepSeek 或本地规则生成的策略评价。
...
| `DEEPSEEK_TOKEN` | DeepSeek API token，用于策略 AI 复盘 |
