...
from scripts.research.analyze_concept_entry_edges import rank_kpl_concepts
from scripts.research.build_moneyflow_cache import context_entry_dates, existing_cache_dates, needed_prior_dates, query_trade_dates
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a run-local point-in-time KPL concept rank cache.")
    ...
    parser.add_argument("--lookback-days", type=int, default=1, help="Previous trading dates to cache for each allowed entry date.")
    parser.add_argument("--sleep-seconds", type=float, default=0.12, help="Sleep between Tushare kpl_concept_cons calls.")
    ...
    cache_path = run_dir / "concept-cache.jsonl"
        ...
        for item in needed_dates:
            ...
            try:
                ...
                for ts_code, metrics in ranked.items():
                    ...
                    saved += 1
                handle.flush()
                per_date.append({"tradeDate": item.isoformat(), "status": "fetched", "rowsSaved": saved, "conceptRows": 0 if df is None else len(df), "rankedStocks": len(ranked)})
...
def render_review(summary: dict[str, Any]) -> str:
    lines = [
        f"# {summary['runId']} KPL concept cache",
        "",
    ...
    if summary["fetchErrors"]:
        ...
        lines.append("")
    lines.extend(["## Use", "", f"`--concept-cache {summary['cachePath']}`", ""])
    return "\n".join(lines)
