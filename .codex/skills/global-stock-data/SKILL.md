...
- 本技能只为离线数据观察、实验与研究提供只读输入；除非另有已批准的正式数据合同，所有结果固定视为 `researchEligible=false`。
- 分析师评级与目标价只能作为带来源的市场观察，不得转换为本仓库的研究结论或建议。
- 不得输出买入、卖出、持有、评级或实盘指令，不得连接账户、券商或执行任何真实资金动作。
- 技术指标、期权、涨跌幅排名和筛选结果不得单独解释为策略有效、收益承诺或正式研究通过。
...
def stock_search(keyword: str, count: int = 10) -> list[dict]:
    ...
    if not token:
        raise RuntimeError("缺少 EASTMONEY_SEARCH_TOKEN")
