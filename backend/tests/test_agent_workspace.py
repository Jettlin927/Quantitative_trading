        ...
        return DomainToolResult.success(
            ...
            evidence=(
                EvidenceEnvelope(
                    ...
                    source="portfolio",
                    as_of=NOW,
                    content_sha256=f"{calls['count']:064x}",
    ...
    def test_persisted_tool_evidence_excerpts_are_authorized_per_envelope(self) -> None:
        def holdings(_context, _arguments):
            return DomainToolResult.success(
                ...
                evidence=(
                    EvidenceEnvelope(
                        "ledger:item:1", "source-1", NOW, "1" * 64, ("holdings",)
                    ),
                    EvidenceEnvelope(
                        "ledger:item:2", "source-2", NOW, "2" * 64, ("usd_cash",)
                    ),
                ),
            )
        ...
        workspace = make_workspace(provider, holdings_handler=holdings)
        receipt = workspace.prepare(
            ACTOR,
            AnalysisIntent(question="NVDA 怎么看？", subject_ids=("NVDA",)),
