...
自动简报不复用用户确认动作：Worker 先用固定配方执行服务端只读工具，冻结证据与缺口，
再以持久 trigger 和日预算预留保证同一 XNYS 时段或结构化事件最多收费一次。模型只做一次
Completion；provider 调用开始后若结果未知则停止重试并保守记账。Hosted Web Search、跨供应商
fallback 与任何交易动作都不在这条路径中。
