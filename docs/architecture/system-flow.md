...
4. 私有 Worker 按 XNYS 交易日历在盘前（美东 04:00 起）、盘中（实际开盘起）和盘后（实际收盘起）各触发一次“活跃持仓标的 ∩ 已启用规则标的”检查；节假日不运行，提前收盘按当日真实收盘时间切换，20:00 至次日 04:00 不运行。同一交易日与时段保持幂等。移出持仓后不再自动评估，也不再把旧命中投影到今日页。
