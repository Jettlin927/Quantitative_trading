...
| `/srv/quantitative-trading/research-artifacts` | 遗留工件目录；清理前只保护、不新增 | 运行用户 `0750` |
...
服务器为 2 核、3.6GiB 内存和 2GiB swap。当前常驻资源由 PostgreSQL、API 和前端合计，并给私有 `personal-analysis-worker` 留出余量：
...
镜像构建、数据恢复和 migration 不得并行。Compose 的 `cpus`、`mem_limit` 和 `pids_limit` 语义以 [Compose services 规范](https://docs.docker.com/reference/compose-file/services/)为准。容器与 daemon 日志均使用 `json-file`，单文件 10MiB、最多 3 份；配置依据 [Docker 日志轮转说明](https://docs.docker.com/engine/logging/drivers/json-file/)。
...
- 合并配置读回为 PostgreSQL `127.0.0.1:5432`、API `127.0.0.1:18000`、前端 `127.0.0.1:15173`；两处持久挂载分别指向 PostgreSQL 与现已遗留的工件目录。该历史验收不代表当前 Compose 仍应包含旧服务。
