...
本仓库使用本地 Git、GitHub Actions 和目标服务器 Compose，但三者代表不同事实：代码已推送、CI 通过、镜像构建、容器启动、生产读回和研究结论不得混写。
...
5. 读回镜像、容器、API、已启用的研究/个人分析 worker、前端、schema 与数据事实。
6. 保留回滚源码、数据库备份和 canonical 工件；未经批准不清理。

## 禁止项

- GitHub Actions 不持有生产 SSH 私钥，也不执行依赖生产数据的正式研究。
- API 启动不自动执行 Alembic migration。
- 部署脚本不得删除 PostgreSQL volume、备份或 canonical 研究工件。
