...
`GET /api/research/publications/{publication_id}/analytics` 只读取该发布绑定的评价、运行和冻结证据，返回累计净收益、CAGR、匹配基准收益、超额或相对收益、回撤、风险调整、尾部、交易、成本、暴露与容量指标，并在 canonical 账本存在时返回净值、基准、回撤、换手和成本序列。平均单边换手、累计单边换手与累计成本率使用不同字段，不能混用同一来源值；walk-forward、参数邻域、成本压力、DSR/PBO 与容量分别返回证据状态和缺失原因。逐年结果与方向 × 波动率市场环境矩阵也绑定同一评价版本。前端只负责格式化和绘图，不重新计算研究指标。

历史导入仅对冻结迁移清单中的四项当前研究使用显式适配器，并在读取时校验来源 SHA-256；不按字段相似度猜测。`legacy` 档案固定显示“仅追溯，未按当前标准评价”。缺少匹配基准、账本或任何指标时返回 `not_available` 和原因，不以 `0`、空折线或成功运行状态代替证据。该动态投影不覆盖已发布的 `summary.json`、`report.html` 或 manifest，也不改变评价结论。
