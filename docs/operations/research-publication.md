...
`GET /api/research/publications/{publication_id}/analytics` 只读取该发布绑定的评价、运行和冻结证据，返回累计净收益、CAGR、匹配基准收益、超额或相对收益、回撤、风险调整、尾部、交易、成本、暴露与容量指标，并在 canonical 账本存在时返回净值、基准、回撤、换手和成本序列。逐年结果与方向 × 波动率市场环境矩阵也绑定同一评价版本。前端只负责格式化和绘图，不重新计算研究指标。
