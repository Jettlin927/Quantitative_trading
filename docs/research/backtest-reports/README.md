...
未完全关闭最终验收第 2 项：`05_stoploss_trend_filter` 只有趋势过滤行级结果，缺止损三档独立报告；`12_kronos_forecast_slope` 只有信号函数和单测，缺滚动历史预测回测。
...
| `05_stoploss_trend_filter` | 止损与趋势过滤 | 部分覆盖 | `base_strategy_comparison.csv` 中有 `ram_top2_m60_v60_monthly_trend_filter_cost` 行 | 有趋势过滤对比，但未找到 `5%/10%/15%` 止损三档独立报告；最终验收仍需补。 |
...
| `12_kronos_forecast_slope` | Kronos 预测斜率信号 | 缺历史回测 | `experiment/kronos_forecast_slope.py`；`tests/test_kronos_forecast_slope.py` | 已有信号函数和单测，但未找到滚动历史预测回测结果；最终验收需补。 |
...
1. 为 `05_stoploss_trend_filter` 补止损三档独立结果，至少输出 summary 和 CSV。
2. 为 `12_kronos_forecast_slope` 补滚动历史预测回测；如果缺历史 Kronos 预测数据，应先标明数据缺口，不能伪造回测。
3. 将本索引中的可引用证据接入 `my_quant/us_research/reports/latest_us_operations.html`，让美股关注标的报告能引用策略或规则证据。
